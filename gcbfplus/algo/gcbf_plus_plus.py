...
class GCBFPlus(GCBF):
    ...
    def __init__(
        ...
        actor_optim = optax.adamw(learning_rate=lr_actor, weight_decay=1e-3)  # 创建一个使用 AdamW 算法、指定学习率和权重衰减的优化器，并将其存储在 actor_optim 中
        ...
        # set up key
        self.key = key
        self.buffer = MaskedReplayBuffer(size=buffer_size)  # 512
        self.unsafe_buffer = MaskedReplayBuffer(size=buffer_size // 2)  # 256
    ...
    def safe_mask(self, unsafe_mask: Array) -> jnp.ndarray:  #! 目前的horizon是32，是否意味着32步安全则认为是安全的
    ...
    def step(self, graph: GraphsTuple, key: PRNGKey, params: Optional[Params] = None) -> Tuple[Action, Array]:
        ...
        return 2 * action + self._env.u_ref(graph), log_pi  #! 这里*2是什么意思
    ...
    def update_nets(self, rollout: Rollout, safe_mask, unsafe_mask):  #! 重要 用于更新网络
        update_info = {}

        # Compute b_u_qp.
        n_chunks = 8
        batch_size = len(rollout.graph.states)  # 4096 到 12288 然后保持不变 设置n=4之后 graph的数量得到显著下降 现在就是看这个graph为啥这么大
        chunk_size = batch_size // n_chunks  # 8 到 1536

        # t0 = time.time()
        b_u_qp = []  # 12288 * 4 * 2
        for ii in range(n_chunks):
            graph = jtu.tree_map(lambda x: x[ii * chunk_size: (ii + 1) * chunk_size], rollout.graph)
            b_u_qp.append(jax2np(self.get_b_u_qp(graph, self.cbf_tgt.params)))  # 消耗显存  4个的时候 n_graph=1536
        b_u_qp = tree_merge(b_u_qp)

        batch_orig = Batch(rollout.graph, safe_mask, unsafe_mask, b_u_qp)

        for i_epoch in range(self.inner_epoch):  #! 每次更新进行8次梯度下降 inner_epoch=8
    ...
    def update(self, rollout: Rollout, step: int) -> dict:
        ...
        rollout_np = jax2np(rollout)  # 将 JAX 数组转换为 NumPy 数组
        del rollout
    ...
    def get_qp_action(
        ...
        u_ref = self._env.u_ref(graph).reshape(-1)  #! 每个env都有一个u_ref 在double_integrator中是LQR控制器
    ...
    def update_inner(
        ...
        def update_fn(carry, minibatch: Batch):
            ...
            def get_loss(cbf_params: Params, actor_params: Params) -> Tuple[Array, dict]:
                ...
                loss_h_dot = jnp.mean(max_val_h_dot)  # 相当于 jax.numpy.mean
            ...
            #  jax.value_and_grad 函数，它用于计算一个函数的值和梯度
            (loss, loss_info), (grad_cbf, grad_actor) = jax.value_and_grad(  #! 得到loss的value和相对于cbf和actor参数的梯度
                get_loss, has_aux=True, argnums=(0, 1))(cbf.params, actor.params)
            grad_cbf, grad_cbf_norm = compute_norm_and_clip(grad_cbf, self.max_grad_norm)  #! 计算梯度的范数并进行裁剪
            grad_actor, grad_actor_norm = compute_norm_and_clip(grad_actor, self.max_grad_norm)
            cbf = cbf.apply_gradients(grads=grad_cbf)  #! 更新cbf和actor的参数
        ...
        (cbf_train_state, actor_train_state), info = lax.scan(update_fn, train_state, batch)  # JAX 库的 lax.scan 函数。lax.scan 是一个高效的循环原语，它可以用于替代 Python 的内置循环，以提高性能 对 batch 中的每个元素执行 update_fn 函数，并在每次循环中更新 train_state
